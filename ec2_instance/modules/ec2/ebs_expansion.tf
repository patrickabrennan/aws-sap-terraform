########################################
# Builds local.normalized_disks
########################################

locals {
  app = lower(var.application_code)

  # ---------- Common disks per app ----------
  defaults_common_map = {
    hana = [
  ...
  # Base specs (tune sizes as needed)
  ...
  # Normalize: unique sequential index and safe fields
  normalized_disks = [
